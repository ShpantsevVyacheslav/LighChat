...
/**
 * React hook to subscribe to a Firestore collection or query for real-time updates.
 *
...
export function useCollection<T = any>(
      ...
      unsubscribeCombined = scheduleFirestoreListen(() =>
        onSnapshot(
          ...
          (err: FirestoreError) => {
            ...
            const firestore = (memoizedTargetRefOrQuery as { firestore?: Firestore }).firestore;
            if (err.code === 'permission-denied') {
              logFirestorePermissionDenied({
                source: 'useCollection',
                operation: 'list (onSnapshot)',
                path,
                firestore: firestore ?? null,
                extra: {
                  refType: memoizedTargetRefOrQuery.type,
                },
                error: err,
              });
            }
  ...
  }, [memoizedTargetRefOrQuery]); // Re-run if the target changes.
