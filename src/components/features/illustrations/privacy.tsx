    ...
    <div className={cn('relative flex h-full w-full flex-col gap-2 p-3', className)}>
      <div className="flex items-center gap-2 rounded-2xl border border-primary/20 bg-primary/10 px-3 py-2">
        ...
        <div className="leading-tight">
          <p className="text-xs font-semibold text-foreground">{t.privacyTitle}</p>
          <p className="text-[10px] text-muted-foreground">{t.privacySubtitle}</p>
        </div>
      </div>
      <div className="grid flex-1 gap-1.5 overflow-hidden">
        <AnimatedSwitchRow label={t.privacyOnline} hint={t.privacyOnlineHint} on delayMs={0} />
        <AnimatedSwitchRow label={t.privacyLastSeen} hint={t.privacyLastSeenHint} on={false} animateToggle delayMs={120} />
        <AnimatedSwitchRow label={t.privacyReceipts} hint={t.privacyReceiptsHint} on delayMs={240} />
        {!compact ? (
          <>
            <AnimatedSwitchRow label={t.privacyGlobalSearch} hint={t.privacyGlobalSearchHint} on={false} delayMs={360} />
            <AnimatedSwitchRow label={t.privacyGroupAdd} hint={t.privacyGroupAddHint} on delayMs={480} />
          </>
        ) : null}
      </div>
    </div>
