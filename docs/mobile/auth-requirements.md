...
- **username**
  - trim
  - allowed: латиница/цифры/`_`/`.`; допускает префикс `@` (удаляется при нормализации); если введён email — для автогенерации берётся часть до `@`
  - min length: 3, max length: 30
  - см. `src/lib/username-candidate.ts` / `normalizeUsernameCandidate` + `isNormalizedUsernameTokenAllowed`
- **phone**
...
- `name.trim().length >= 2`
- `username` после нормализации (`normalizeUsernameCandidate` в web / пакете `lighchat_firebase`):
  - `3..30`
  - допустимые символы: `[a-z0-9._]`, без `..`, не начинается/не заканчивается точкой
