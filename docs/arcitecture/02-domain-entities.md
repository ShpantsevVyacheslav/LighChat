...
- `GameSession` - сессия мини-приложения внутри чата (напр. “Дурак”): хранится в `games/{gameId}` и создаётся сервером (Cloud Functions). Доступ на чтение — только участникам (uid ∈ `playerIds`).
  - Для “Дурака” игра имеет публичное представление `publicView` (фаза раунда, стол, счётчики рук) и приватные документы рук `privateHands/{uid}`.
  - Результат завершённой партии фиксируется в `result` + `finishedAt`.
