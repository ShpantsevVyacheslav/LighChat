...
- `mobile/app` - Flutter приложение (iOS/Android) — новый клиент для стора.
  ...
  - `lib/features/chat/ui/chat_ios_image_markup.dart`, `ios/Runner/AppDelegate.swift` (`lighchat/image_markup`) - iOS-hybrid редактирование фото в композере: для одиночного pending-image вызывается native Markup editor (PencilKit tool picker + сохранение JPEG), для альбомов и fallback остаётся `ChatImageEditorScreen`.
