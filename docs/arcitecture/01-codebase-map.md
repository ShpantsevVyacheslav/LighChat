...
- `mobile/app` - Flutter приложение (iOS/Android) — новый клиент для стора.
  ...
  - `lib/features/chat/ui/chat_document_open.dart`, `lib/features/chat/ui/chat_pdf_viewer_screen.dart`, `ios/Runner/AppDelegate.swift` (`lighchat/document_preview`) - открытие PDF/текстовых документов внутри приложения: iOS-native Quick Look preview (системный Markup/Sign), Android встроенный PDF viewer с text-markup аннотациями и экспортом (`syncfusion_flutter_pdfviewer`), fallback на `LinkWebViewScreen` для web-ссылок.
  - `lib/features/chat/ui/chat_ios_image_markup.dart`, `ios/Runner/AppDelegate.swift` (`lighchat/image_markup`) - iOS-hybrid редактирование фото в композере: тап по карандашу на pending-image всегда открывает `ChatImageEditorScreen`; уже внутри редактора кнопка карандаша на iOS поднимает `QLPreviewController` с `editingMode = .createCopy` — Apple сама рисует полный системный Markup (карандаш/маркер/перо/ластик/лассо + «+» меню со стикерами / текстом / подписью / фигурами / лупой / описанием), сохранённую копию мы получаем через `previewController(_:didSaveEditedCopyOf:at:)` и копируем в свой temp-каталог. На Android та же кнопка — встроенный Flutter draw-mode. Caption/crop/multi-image остаются в `ChatImageEditorScreen` на обеих платформах.
