...
- `mobile/app` - Flutter приложение (iOS/Android) — новый клиент для стора.
  ...
  - `lib/features/chat/ui/chat_ios_image_markup.dart`, `ios/Runner/AppDelegate.swift` (`lighchat/image_markup`) - iOS-hybrid редактирование фото в композере: тап по карандашу на pending-image всегда открывает `ChatImageEditorScreen`; уже внутри редактора кнопка карандаша на iOS вызывает native Markup editor (PencilKit tool picker + сохранение JPEG), а на Android — встроенный Flutter draw-mode. Caption/crop/rotate/multi-image остаются в `ChatImageEditorScreen` на обеих платформах.
