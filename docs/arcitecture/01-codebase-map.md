...
- `mobile/app` - Flutter приложение для **iOS / Android / macOS / Windows / Linux**. Единая кодовая база; платформо-зависимое поведение изолировано в `lib/platform/platform_capabilities.dart`.
  ...
  - `lib/features/settings/ui/storage_settings_screen.dart`, `lib/features/settings/data/storage_cache_manager.dart` - экран «Хранилище» с разбивкой кэша по чатам/файлам/категориям, очисткой (весь кэш/по чату/по категории/по файлу), лимитом объёма и политиками локального хранения. Категории кэша (enum `LocalStorageCategory`): `e2eeMedia`, `e2eeText`, `chatDrafts`, `chatListSnapshot`, `profileCards`, `videoDownloads`, `videoThumbs`, `chatImages`, `stickersGifsEmoji` (агрегирует `recent_stickers_v1` + `giphy_query_cache_v2` + `giphy_recent_gifs_v1` в SharedPreferences), `networkImageCache` (`${tempDir}/libCachedImageData/` без подкаталога `chat_image_cache/`). Типы медиа в donut/breakdown (`StorageMediaType`): `video`/`photo`/`audio` (`.m4a/.mp3/.ogg/.wav/.aac/.opus/.oga` — голосовые сообщения)/`file`/`other`. Список чатов включает обычный, секретный (через `userSecretChatIndexProvider` + `userSecretChatFallbackIdsProvider`) индексы. Файлы кэша без `conversationId` (orphan) рендерятся отдельной строкой «Без привязки к чату» (`kStorageOrphanConversationId == '__orphan__'`) с возможностью очистки.
  - `lib/features/settings/ui/energy_saving_screen.dart`, `lib/features/settings/data/energy_saving_preference.dart` - экран «Энергосбережение» (паритет с Telegram): слайдер порога заряда (Off/5/10/15/20/25/Always) + 7 тумблеров ресурсоёмких процессов (autoplay video/GIF, animated stickers/emoji, interface animations, media preload, background update). `EnergySavingNotifier` (riverpod) персистит флаги в `SharedPreferences` под ключами `energySaving.*`, слушает `battery_plus.onBatteryStateChanged` и `isInBatterySaveMode` (Android Power Save / iOS Low Power Mode). Геттер `isLowPowerActive` = (системный saver) ИЛИ (порог достигнут) ИЛИ (`always`); `effective*`-геттеры возвращают `false`, когда `isLowPowerActive`. Точки интеграции в feature-коде: `MessageVideoAttachment` (autoplay + preload cache warm-up), `MessageAttachments`/`ComposerStickerGifPanel`/`ComposerStickerSuggestionRow` (TickerMode-гейтинг GIF/sticker/animated-emoji, плюс отключение части implicit-анимаций при `effectiveInterfaceAnimations=false`), `_ChatMessageListState._triggerEmojiBurst` (пропуск burst), `ChatListScreen` (`E2eePlaintextCache.warmUpPreviews` только при `effectiveMediaPreload=true`), `app_providers.dart` (`chatRealtimeEnabledProvider` + `appLifecycleStateProvider` отключают realtime-stream’ы в фоне при `effectiveBackgroundUpdate=false`). Точка входа в меню — `chat_account_screen.dart`, пункт `account_menu_energy_saving`. Маршрут `/settings/energy-saving` в `app_router.dart`.
  ...
  - `lib/features/chat/ui/chat_document_open.dart`, `lib/features/chat/ui/chat_pdf_viewer_screen.dart`, `lib/features/chat/ui/message_attachments.dart`, `ios/Runner/AppDelegate.swift` (`lighchat/document_preview`) - открытие документов внутри приложения и Telegram-style document preview в пузыре сообщения (PDF/DOC/XLS/PPT/TXT/RTF): iOS-native Quick Look preview + системная thumbnail-генерация первой страницы (QuickLookThumbnailing), Android встроенный PDF viewer с text-markup аннотациями и экспортом (`syncfusion_flutter_pdfviewer`), fallback на `LinkWebViewScreen` для web-ссылок.
  - `lib/features/chat/ui/chat_ios_image_markup.dart`, `ios/Runner/AppDelegate.swift` (`lighchat/image_markup`) - iOS-hybrid редактирование фото в композере: тап по карандашу на pending-image всегда открывает `ChatImageEditorScreen`; уже внутри редактора кнопка карандаша на iOS поднимает `QLPreviewController` с `editingMode = .createCopy` — Apple сама рисует полный системный Markup (карандаш/маркер/перо/ластик/лассо + «+» меню со стикерами / текстом / подписью / фигурами / лупой / описанием), сохранённую копию получаем через `previewController(_:didSaveEditedCopyOf:at:)` и копируем в temp-каталог. На Android та же кнопка — встроенный Flutter draw-mode. Caption/crop/multi-image остаются в `ChatImageEditorScreen` на обеих платформах.
  ...
  - `lib/features/chat/ui/chat_drag_drop_target.dart`, `lib/features/chat/data/composer_clipboard_paste.dart` - системный drag&drop в `ChatScreen`/`ThreadScreen` (пакет `super_drag_and_drop`): `DropRegion` со `Formats.standardFormats` + overlay-подсказка; payload читается через общую `readComposerPayloadFromDataReaders(Iterable<DataReader>)`, которой пользуется и paste из буфера. Файлы попадают в `_pendingAttachments`, текст — в позицию курсора композера; дальше — обычный `uploadChatAttachmentFromXFile` pipeline.
  - `lib/features/chat/data/sticker_drop_heuristic.dart` - эвристика «это стикер, а не фото» для drag&drop: PNG/WebP magic, размер ≤ 512px, ≤ 1 MiB, прозрачные углы (alpha < 16). На срабатывании `_handleDroppedFiles` отправляет файл через `_sendStickerOrGifAttachment` (chat) / `_sendThreadStickerOrGifAttachment` (thread) — Recent stickers + E2EE envelope из коробки.
  - `lib/features/chat/data/share_intent_payload.dart`, `lib/features/chat/data/share_intent_listener.dart`, `lib/features/chat/ui/share_target_picker_screen.dart`, `ios/ShareExtension/ShareViewController.swift`, `ios/scripts/add_share_extension_target.rb` - системный «Поделиться → LighChat» (пакет `receive_sharing_intent`). Android: intent-filter `ACTION_SEND(_MULTIPLE)` в `AndroidManifest.xml`, `launchMode=singleTask`. iOS: App Extension target `ShareExtension` (наследник `RSIShareViewController`, `shouldAutoRedirect=true`), App Group `group.com.lighchat.lighchatMobile` в Runner+Extension entitlements. Flutter side: `ShareIntentListener` в `MyApp.initState` ловит payload и делает `go('/share', extra: ShareIntentPayload)`; `ShareTargetPickerScreen` показывает список существующих чатов (через тот же `buildForwardRecipientRows`); тап → `pushReplacement('/chats/{id}', extra: payload)`; `ChatScreen` через optional `initialSharePayload` кладёт файлы в `_pendingAttachments` и вставляет текст в композер.
