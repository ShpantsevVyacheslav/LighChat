...
- **Аналитика в BigQuery (`firestore-bigquery-export@0.3.2`):** Firebase Extension зеркалит коллекцию `analyticsEvents` (см. [src/lib/analytics/](src/lib/analytics/) и [src/app/api/analytics/event/route.ts](src/app/api/analytics/event/route.ts)) в `project-72b24.firestore_export.analytics_events_raw_changelog` (table, partition by DAY) + `analytics_events_raw_latest` (view). Конфиг extension зафиксирован в [extensions/firestore-bigquery-export.env](extensions/firestore-bigquery-export.env) и [firebase.json](firebase.json) для воспроизводимости. SQL для Looker Studio dashboard'ов — [docs/arcitecture/analytics-bigquery-queries.md](docs/arcitecture/analytics-bigquery-queries.md). Параллельно события идут в GA4 stream `lighchat.online` (G-YG1C6DD3TC) через web SDK — используется как secondary tool для marketing-команды.
