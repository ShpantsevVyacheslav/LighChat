 ...
 * Дублируется в `src/lib/push-notification-policy.ts` для клиента — правки вносить в оба файла.
 */
...
/** Текущее время суток в минутах 0..1439 в указанной IANA-зоне. */
...
/**
 * Решение по доставке push о новом сообщении в чате.
 */
...
export function buildDataPayload(parts: {
  title: string;
  body: string;
  link: string;
  tag: string;
...
export function stableDataKey(data: Record<string, string>): string {
