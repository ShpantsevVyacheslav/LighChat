  ...
  void _publish() {
    ...
    if (q.isEmpty) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.poll_create_enter_question)));
      return;
    }
    if (opts.length < 2) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.poll_create_min_options)));
      return;
    }
    if (_quiz) {
    ...
    if (_closesAt != null && !_closesAt!.isAfter(DateTime.now())) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l10n.poll_create_future_time)));
      return;
    }
                          ...
                          children: [
                            ...
                            if (_quiz && opts.length >= 2) ...[
                              ...
                              DropdownButtonFormField<int>(
                                ...
                                decoration: InputDecoration(
                                  labelText:
                                      l10n.poll_create_correct_option_label,
                                ),
                                initialValue: _correctIdx.clamp(
                                  0,
                                  opts.length - 1,
                                ),
                                items: [
